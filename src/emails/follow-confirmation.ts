...
/**
 * Sends a one-time "You're now following X" confirmation email.
 * Called after successful follow of a category or meeting.
 */
export async function sendFollowConfirmationEmail({
  ...
  name,
  appUrl,
}: SendFollowConfirmationParams): Promise<{ id: string | null; error: unknown }> {
  const subject =
    type === "category"
