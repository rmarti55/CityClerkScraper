...
const DEFAULT_DEPT_COLOR = { bg: "bg-gray-50", text: "text-gray-800", border: "border-gray-200" };
        ...
        {person.email && (
          <div className="flex items-center gap-2 text-xs text-gray-700 group">
            <a
              ...
              className="flex items-center gap-2 min-w-0 hover:text-teal-700 transition-colors"
            >
              <svg className="w-3.5 h-3.5 text-gray-700 group-hover:text-teal-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
              <span className="truncate">{person.email}</span>
            </a>
            <CopyButton value={person.email} label="Email copied" />
          </div>
        )}
        {person.phone && (
          <div className="flex items-center gap-2 text-xs text-gray-700 group">
            ...
            <CopyButton value={person.phone} label="Phone copied" />
          </div>
        )}
      </div>
    </div>
          ...
          {Array.from(grouped.entries()).map(([letter, persons]) => (
            <div key={letter}>
              <div className="sticky top-0 z-10 bg-gray-50 py-1 mb-3">
                <h2 className="text-lg font-bold text-gray-400">{letter}</h2>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {persons.map((p) => (
                  <PersonCard
